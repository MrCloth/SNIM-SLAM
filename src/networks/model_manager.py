...
from src.networks.sam2_seg import SAM2SEG
...
#original name is get_
    def get_dinov2(self):
        ...
        return model
    def get_sam2(self):
        model = SAM2SEG(img_h=self.img_h, img_w=self.img_w, num_cls=self.n_classes, edge=self.crop_edge, dim=self.dim)
        model.load_state_dict(torch.load(self.pretrained_model_path, map_location=self.device))
        return model
